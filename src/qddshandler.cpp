...
static int faceFlags[6] = {DDSHeader::Caps2CubeMapPositiveX,
...
static const FormatInfo formatInfos[] = {{FormatA8R8G8B8, DDSPixelFormat::FlagRGBA, 32, 0x00ff0000, 0x0000ff00, 0x000000ff, 0xff000000},
...
static const Format knownFourCCs[] = {FormatA16B16G16R16, FormatV8U8,          FormatUYVY,  FormatR8G8B8G8, FormatYUY2,          FormatG8R8G8B8,
                                      FormatDXT1,         FormatDXT2,          FormatDXT3,  FormatDXT4,     FormatDXT5,          FormatRXGB,
                                      FormatATI2,         FormatQ16W16V16U16,  FormatR16F,  FormatG16R16F,  FormatA16B16G16R16F, FormatR32F,
                                      FormatG32R32F,      FormatA32B32G32R32F, FormatCxV8U8};
...
static const FormatName formatNames[] = {{FormatUnknown, "unknown"},
...
{
    return qRgb(quint8(Y + 1.13983 * (V - 128)), quint8(Y - 0.39465 * (U - 128) - 0.58060 * (V - 128)), quint8(Y + 2.03211 * (U - 128)));
}
...
static Format getFormat(const DDSHeader &dds)
{
    ...
    if (format.flags & DDSPixelFormat::FlagPaletteIndexed4) {
        ...
        for (size_t i = 0; i < formatInfosSize; ++i) {
            ...
            if ((format.flags & info.flags) == info.flags && format.rgbBitCount == info.bitCount && format.rBitMask == info.rBitMask
                && format.gBitMask == info.gBitMask && format.bBitMask == info.bBitMask && format.aBitMask == info.aBitMask) {
                return info.format;
            }
        }
    }
...
{
    QImage::Format format = (version == Two || version == Four) ? QImage::Format_ARGB32_Premultiplied : QImage::Format_ARGB32;
...
bool QDDSHandler::read(QImage *outImage)
{
    ...
    QImage image = isCubeMap(m_header) ? readCubeMap(s, m_header, m_format) : readTexture(s, m_header, m_format, m_currentImage);
...
bool QDDSHandler::write(const QImage &outImage)
{
    ...
    dds.magic = ddsMagic;
    dds.size = 124;
    dds.flags = DDSHeader::FlagCaps | DDSHeader::FlagHeight | DDSHeader::FlagWidth | DDSHeader::FlagPixelFormat;
...
{
    return (option == QImageIOHandler::Size) || (option == QImageIOHandler::SubType) || (option == QImageIOHandler::SupportedSubTypes);
}
...
bool QDDSHandler::verifyHeader(const DDSHeader &dds) const
{
    ...
    quint32 requiredFlags = DDSHeader::FlagCaps | DDSHeader::FlagHeight | DDSHeader::FlagWidth | DDSHeader::FlagPixelFormat;
    ...
    if (dds.size != ddsSize) {
        qWarning() << "Wrong dds.size: actual =" << dds.size << "expected =" << ddsSize;
        return false;
    }

    if (dds.pixelFormat.size != pixelFormatSize) {
        qWarning() << "Wrong dds.pixelFormat.size: actual =" << dds.pixelFormat.size << "expected =" << pixelFormatSize;
        return false;
    }
